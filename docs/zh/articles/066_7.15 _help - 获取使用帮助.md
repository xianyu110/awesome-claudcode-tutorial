...
`[command]`| 可选，指定要查看的命令名称。如果不提供，则显示所有命令
...
## 最佳实践
  ...
  1. **新手必用** ：首次使用时，先运行 `/help` 了解所有命令
  2. **忘记用法** ：忘记某个命令的用法时，使用 `/help <command>` 查看
  3. **定期查看** ：定期查看帮助，了解新功能和命令
  4. **探索功能** ：通过 `/help` 发现你可能不知道的功能
